            ...
            <tbody>
              {contractList.map((contract, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{contract.contrId}</td>
                  <td>{contract.status}</td>
                  <td>{contract.updater}</td>
                  <td>
                    <TextFormat type="date" value={contract.updateTimeStamp} format={APP_DATE_FORMAT} />
                  </td>
                  <td>{contract.businessUnit}</td>
                  <td>
                    {contract.rtSchedRateScheduleCD ? (
                      <Link to={`rate-sched/${contract.rtSchedId}`}>{contract.rtSchedRateScheduleCD}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>
                    {contract.busAssocBaAbbr ? <Link to={`business-associate/${contract.busAssocId}`}>{contract.busAssocBaAbbr}</Link> : ''}
                  </td>
